...
module.exports = client => {
    // Add your YouTube logging code here
    console.log("YouTube logging initialized.");
    ...
    async function check(client){
        console.log("[YOUTUBE] :: Checking Accounts...".italic.brightRed)
        client.guilds.cache.map(guild => guild.id).forEach(guildid => {
            try{
                ...
                client.social_log.get(guildid, "youtube.channels").forEach(async (youtuber) => {
                    console.log(`[YOUTUBE] :: [${youtuber}] | Start checking...`.italic.brightRed);
                    let channelInfos = await channelInfo(youtuber);
                    if(!channelInfos) return console.log(`[YOUTUBE] :: [ERR] | Invalid youtuber provided: ${youtuber}`.italic.brightRed);
                    //channelInfos.url, ChannelDATA
                    let video = await checkVideos(channelInfos.url, youtuber);
                    if(!video) return console.log(`[YOUTUBE] :: [${channelInfos.name}] | No notification`.italic.brightRed);
                    
                    let channel = await client.channels.fetch(client.social_log.get(guildid, "youtube.dc_channel")).catch(e=>{return;})
                    if(!channel) return console.log("[YOUTUBE] :: [ERR] | DC-Channel not found".italic.brightRed);
                    ...
                    console.log("[YOUTUBE] :: Notification sent!".italic.brightRed);
                });        
            }catch (e){
                console.log(`[YOUTUBE] :: ${e}`.grey)
            }
        })
    }
