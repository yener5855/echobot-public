...
module.exports = client => {
  ...
  // Add your Twitter feed logging code here
  console.log("Twitter feed logging initialized.");
}
async function create_twit(client){
      console.log("[TWITTER] :: Checking Accounts".cyan)
      ...
      if(!userids || userids.length == 0) return console.log(`[TWITTER] :: NO USERIDS: ${userids}`.cyan)
      //create a new TWIT
      var T = new Twit({
      ...
      console.log(`[TWITTER] :: ${userids.filter(i=> i?.toLowerCase() != "impressgm").join(", ")}`.cyan)
      for(const user of userids){
        if(String(user).toLowerCase().includes("impressgm")) continue;
        console.log(`[TWITTER] :: Checking: ${user}`.cyan);
        gettwit(user);
        await delay(1500);
      }
        ...
        try{
          await T.get('search/tweets', { q: `from:${user}`, count: 10 }, function(err, data, response) {
            try{
              ...
              if (!data || !data.statuses[0] || !data.statuses[0].user) return console.log("COULD NOT FIND THE DATA".cyan);
              ...
              if(!guildid || guildid == null || guildid == undefined || guildid.length != 18) return console.log("[TWITTER] :: NO VALID GUILD".cyan)
              try{
              ...
              if(twitsettings.latesttweet === tweet.id_str) return console.log("[TWITTER] :: LATEST TWEET".cyan)
              ...
              return console.log(`[TWITTER] :: NOT RIGHT TWEET! :: ${url}`.cyan)
              //If retweet, do that
                ...
                client.channels.fetch(twitsettings.DISCORD_CHANNEL_ID).then(channel => {
                  channel.send(twitsettings.infomsg.replace("{Twittername}", TwitterName).replace("{url}", url)).then(msg=>{
                    ...
                    console.log(`[TWITTER] :: ${e}`.grey)
                  })
                }).catch(err => {
                  console.log(`[TWITTER] :: ${err}`.grey)
                })
