...
const { getVpsStatus, generateMemoryUsageGraph } = require('../../handlers/functions');
module.exports = {
  name: "help",
  category: "🔰 Info",
      ...
      if (args[0]) {
        ...
        let menuOptions = [
          ...
          },
          {
            ...
            value: "Birthday",
            emoji: "🎉",
            description: "Commands for Birthday celebrations"
          },
          {
            label: "Customcommand",
            value: "Customcommand",
      ...
      function allotherembeds_eachcategory(filterdisabled = false) {
        ...
        var embed15 = new MessageEmbed()
          ...
          .addFields(
            {
              name: "\u200b",
              value: "> " + client.commands.filter((cmd) => cmd.category === "🎮 MiniGames").sort((a, b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲")
            }
          )
...
.setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "🎉 Birthday").size}\`] 🎉 Birthday Commands 🎉 | ${settings.BIRTHDAY ? `${customEmojis.general.yes} ENABLED` : `${customEmojis.general.no} DISABLED`}`)
.setDescription(`> *${client.commands.filter((cmd) => cmd.category === "🎉 Birthday").sort((a, b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲")}*`)
.addFields(
  {
    name: "\u200b",
    value: "__**Sub-Categorized Commands:**__"
  },
  {
    ...
    value: `> ${client.commands.filter((cmd) => cmd.category === "🎉 Birthday" && cmd.type === "manage").sort((a, b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲")}`
  },
  {
    name: "🎈 **Birthday Information**",
    value: `> ${client.commands.filter((cmd) => cmd.category === "🎉 Birthday" && cmd.type === "info").sort((a, b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲")}`
  }
);
if (!filterdisabled || settings.BIRTHDAY || settings.showdisabled) embeds.push(embed17)
...
/**
 * @INFO
 * Bot Coded by yener5855#6966 | https://discord.gg/nxJNFAyKwT
 * @INFO
 * Work for yener5855 Development | https://yener5855.eu
 * @INFO
