...
module.exports = {
  ...
  type: "bot",
  description: "Changes if the Advertisement of yener5855-HOST.de Should be there or NOT",
        ...
        let menuoptions = [
          {
            ...
            description: `${client.ad.enabled? "Disables the Ads from yener5855-Host and yener" : "Enables the Ads from yener5855-Host and yener"}`,
        ...
        client.on('interactionCreate',  (menu) => {
          if (menu?.message.id === menumsg.id) {
            if (menu?.user.id === cmduser.id) {
              if(used1) return menu?.reply({content : `<:no:1269533999014084683> You already selected something, this Selection is now disabled!`}, {ephermal : true});
              menuselection(menu);
            }
            else menu?.reply({content : `<:no:1269533999014084683> You are not allowed to do that! Only: <@${cmduser.id}>`}, {ephermal : true});
          }
        });
      }
