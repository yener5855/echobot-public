...
const listBirthdaysCommand = require(`${process.cwd()}/commands/🎉 Birthday/listbirthdays`);
const setBirthdayCommand = require(`${process.cwd()}/commands/🎉 Birthday/setbirthday`);
const deleteBirthdayCommand = require('../../commands/🎉 Birthday/deletebirthday');
const checkBirthdayCommand = require(`${process.cwd()}/commands/🎉 Birthday/checkbirthday`);
const vpsstatus = require('../../commands/👑 Owner/vpsstatus');
...
module.exports = async (client, message) => {
  ...
  try {
    ...
    if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.EMBED_LINKS))
      return message.reply(`<:no:1269533999014084683> **I am missing the Permission to EMBED LINKS (Sending Embeds)**`).catch(()=>{})
    if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.ADD_REACTIONS))
      return message.reply(`<:no:1269533999014084683> **I am missing the Permission to ADD REACTIONS**`).catch(()=>{})
    ...
    if (command && !customcmd) {
      ...
      if(musicData.channel && musicData.channel == message.channel.id){
        return message.reply("<:no:1269533999014084683> **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)}).catch(()=>{})
      }
      if (command.length == 0) {
        if (unkowncmdmessage) {
      ...
      try {
              ...
              if(player && player.queue && player.queue.current && command.parameters.check_dj){
                if(check_if_dj(client, message.member, player.queue.current)) {
                  return message.reply({embeds: [new MessageEmbed()
                    ...
                    .setFooter(client.getFooter(es))
                    .setTitle(`<:no:1269533999014084683> **You are not a DJ and not the Song Requester!**`)
        ...
        //Execute the Command
        console.log(`Command: ${cmd}, Args: ${args}`); // Add this line for debugging
...
client.commands.set(listBirthdaysCommand.name, listBirthdaysCommand);
client.commands.set(setBirthdayCommand.name, setBirthdayCommand);
client.commands.set(deleteBirthdayCommand.name, deleteBirthdayCommand);
client.commands.set(checkBirthdayCommand.name, checkBirthdayCommand);
client.commands.set(vpsstatus.name, vpsstatus);
