...
module.exports = client => {
    ...
    const dirs = [
        ...
        "./databases/stats",
        "./databases/settings",
        "./databases/jointocreatemap",
        "./databases/setups",
        "./databases/queuesaves",
        "./databases/warns",
        "./databases/ranking",
        "./databases/reactionrole",
        "./databases/social_log",
        "./databases/blacklist",
        "./databases/customcommands",
        "./databases/keyword",
        "./databases/birthday"
    ];

    dirs.forEach(dir => ensureDirExists(path.resolve(dir)));
...
/**/client.apply = new Enmap({
    ...
    client.birthdayDB = new Enmap({
        name: "birthdayDB",
        dataDir: "./databases/birthday"
    });
    ...
    client.birthdayDB.ensure("birthdays", {});
    console.log(`[x] :: `.magenta + `LOADED THE DATABASES after: `.brightGreen + `${Date.now() - dateNow}ms`.green)

    // Ensure proper cleanup of resources to avoid memory leaks
    process.on('exit', () => {
        client.notes.close();
        client.economy.close();
        client.invitesdb.close();
        client.tiktok.close();
        client.youtube_log.close();
        client.premium.close();
        client.mutes.close();
        client.snipes.close();
        client.stats.close();
        client.afkDB.close();
        client.musicsettings.close();
        client.settings.close();
        client.jointocreatemap.close();
        client.joinvc.close();
        client.setups.close();
        client.queuesaves.close();
        client.modActions.close();
        client.userProfiles.close();
        client.jtcsettings.close();
        client.roster.close();
        client.autosupport.close();
        client.menuticket.close();
        client.menuapply.close();
        client.apply.close();
        client.points.close();
        client.voicepoints.close();
        client.reactionrole.close();
        client.social_log.close();
        client.blacklist.close();
        client.customcommands.close();
        client.keyword.close();
        client.birthdayDB.close();
    });
}