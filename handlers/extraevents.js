...
module.exports = client => {
  ...
  client.getFooter = (es, stringurl = null) => {
    ...
    if(!text || text.length < 1) text = `${client.user.username} | By:yener#5855`;
  ...
  client.getAuthor = (authorname = null, authoricon = null, authorurl = null) => {
    ...
    if(!name || name.length < 1) name = `${client.user.username} | By:yener#5855`;
    if(!iconURL || iconURL.length < 1) iconURL = `${client.user.displayAvatarURL()}`;
    if(!url || url.length < 1) url = `https://discord.gg/nxJNFAyKwT`;
    ...
    //verify the iconURL
    if(!url.startsWith("https://") && !url.startsWith("http://")) url = `https://discord.gg/nxJNFAyKwT`;
    if(!iconURL.startsWith("https://") && !iconURL.startsWith("http://")) iconURL = client.user.displayAvatarURL();
    if(![".png", ".jpg", ".wpeg", ".webm", ".gif"].some(d => iconURL.toLowerCase().endsWith(d))) iconURL = client.user.displayAvatarURL();
    //return the footerobject
  ...
  */
  ...
  client.on("guildCreate", async guild => {
    ...
    let embed = new MessageEmbed()
      .setColor("GREEN")
      .setTitle(`<a:Join_vc:1269512035066970154> Joined a New Server`)
    ...
    for(const owner of config.ownerIDS){
      //If the Owner is yener5855, and the Bot is in not a yener5855 Development, Public Bot, then dont send information!
      if(owner == "442355791412854784"){
        let yener5855Guild = client.guilds.cache.get("773668217163218944");
        if(yener5855Guild && !yener5855Guild.me.roles.cache.has("779021235790807050")){
          continue; 
        }
      }
