...
module.exports = function (client, options) {
  ...
  async function memberCount(guildid){
    ...
    if(!set) return
    console.log(`Processing guild: ${guild.name}, ID: ${guild.id}`.italic.yellow);
    console.log(`Current setups for this guild: ${JSON.stringify(set)}`.italic.yellow);
    console.log(`Client setups data: ${JSON.stringify(client.setups.get(guild.id))}`.italic.yellow);

    for(let i = 1; i <= 25; i++){
      if(set[`channel${i}`] && set[`channel${i}`].length == 18){
        console.log(`Using channel ID: ${set[`channel${i}`]} for update.`.italic.yellow);
        console.log(`Channel ID ${set[`channel${i}`]} has message: ${set[`message${i}`]}`.italic.yellow);
    ...
    console.log(`Finished processing guild: ${guild.name}, ID: ${guild.id}`.italic.yellow);
  }
  ...
  async function Channel(chId, channelName){
    ...
    try{
        ...
        let newname = String(channelName)
            ...
            .replace(/{offlinemember}/i, guild.members.cache.filter(member => !member.user.bot && member.presence && member.presence.status == "offline").size)
