...
module.exports = client => {
    ...
    client.manager.on("trackError", (player, track, error) => {
        console.error(`Track error in guild ${player.guild} for track ${track.title}:`, error);
    });
    ...
    client.manager.on("queueEnd", (player) => {
        try {
            ...
            if (guild) {
                ...
                if (channel) {
                    channel.send({ embeds: [new MessageEmbed().setColor("RED").setTitle("Queue ended. Leaving the voice channel.")] });
                }
            }
            player.destroy();
        } catch (error) {
            console.error("Error handling queueEnd event:", error);
        }
    });
};
